Describe 'Check class IP is working as expected' {
    BeforeEach {
        import-module -Name $env:USERPROFILE\repos\SCCM-Library\SCCM_Library.psm1 -Force
        $IP1=[IP] "127.0.0.1/8"
...
Describe 'Check class IPRanges is working as expected' {
    BeforeEach {
        import-module -Name $env:USERPROFILE\repos\SCCM-Library\SCCM_Library.psm1 -Force
        $IPRx=[IPRanges]::new()
        $IPRx.AddRange([IPRange] "10.0.0.0-10.255.255.255")
    }
    It 'Check IPRanges type' {
        $IPRx.GetType() | Should -Be "IPRanges"
    }
    It 'Check IPRanges Value' {
        $IPRx | Should -Be "10.0.0.0-10.255.255.255"
    }
}